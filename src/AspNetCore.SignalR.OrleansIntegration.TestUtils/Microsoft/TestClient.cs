...
namespace AspNetCore.SignalR.OrleansIntegration.TestUtils.Microsoft
{
        ...
        class TestClient : ITransferFormatFeature, IConnectionHeartbeatFeature, IDisposable
    {
        ...
        {
            var options = new PipeOptions(readerScheduler: PipeScheduler.Inline, writerScheduler: PipeScheduler.Inline,
                useSynchronizationContext: false);
            var pair = DuplexPipe.CreateConnectionPair(options, options);
            Connection = new DefaultConnectionContext(connectionId ?? Guid.NewGuid().ToString(), pair.Transport, pair.Application);
